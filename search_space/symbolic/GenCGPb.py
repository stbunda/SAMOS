...
class GenCgpb:
    ...
    def __init__(self, conf_net, realgene=None, weights=1):
        """
    ...
    def creategene(self):
        ...
        for n in range(self.conf_net.node_num + self.conf_net.out_num):
            ...
            self.weight[n][0] = np.random.randint(len(self.conf_net.constant_range))  # Randomly choose a kernel # Randomly choose a kernel
    ...
    def active_net_list(self, extra=0):
        ...
        for n, is_active in enumerate(self.is_active):
            if is_active:
                ...
                kernel = self.conf_net.constant_range[weight_kernel]
                ...
                if extra != 0:
                    net_list.append([node_type] + np.add(connections, [extra, extra]).tolist() + [kernel])
                else:
                    net_list.append([node_type] + connections + [kernel])
    ...
    def toReal(self):
        ...
        weight_total = len(self.conf_net.constant_range)
        weight_neg = True if self.conf_net.constant_range[0] < 0 else False
        ...
        for i in range(len(self.gene)):
            ...
            norm_const = (self.weight[i][0] + abs(min(self.conf_net.constant_range))) / (max(self.conf_net.constant_range) + abs(min(self.conf_net.constant_range)))
            self.realgene[i, self.conf_net.max_in_num + 1] = random.uniform(
                norm_const / weight_total,
                (norm_const + 1) / weight_total)

            if np.isnan(self.realgene).any():
                print('help')
